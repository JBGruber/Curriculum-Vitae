...
make_cv <- function(file = "CV_JohannesGruber.Rnw", clean = TRUE) {
  ...
  while (class(check) == "try-error" && length(missing) > 0) {
    ...
    missing <- tinytex::parse_packages(paste0(
  ...
  file.rename("CV_JohannesGruber.pdf", "pdf_version/CV_JohannesGruber.pdf")
  rmarkdown::render("README.Rmd")
}

update_downloads <- function(
  file = "CV_JohannesGruber.bib",
  packages = c(
    "askgpt",
    "atrrr",
    "rwhatsapp",
  ...
  for (i in seq_along(packages)) {
    ...
    entries[[grep(pattern, entries, fixed = TRUE)]] <- gsub(
      "\\{Cranlogs download count: [0-9,]+\\}",
